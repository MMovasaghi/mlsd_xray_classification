...
run = wandb.init(entity='moh2023', project='mlsd', name=f"hw2_hp_run_{ts}")
config = {
    'run_id': ts,
    'batch_size': 640,
    'image_dim': (224, 224),
...
def define_and_train_model(output_to_save_path):
    ...
    model, history = train_model(model, criterion, optimizer, earlyStopper, dataloaders, dataset_size, num_epochs=config['epoch_num'], device=device)

    save_results(model, history, base_path=f"{config['model_path']}")
    cmd = f"cp -r {config['model_path']} {output_to_save_path}"
    os.system(cmd)
